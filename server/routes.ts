...
export function registerRoutes(app: Express): Server {
  ...
  // Get buildings with shift inspectors
  app.get("/api/buildings/shifts", requireAuth, async (req: Request, res: Response) => {
    try {
      // First get all buildings with their supervisors
      const buildingsWithSupervisors = await db
        .select({
          id: buildings.id,
          name: buildings.name,
          area: buildings.area,
      ...
      // For each building, get approved shifts with inspector and shift type details
      const buildingsWithShifts = await Promise.all(
        buildingsWithSupervisors.map(async (building) => {
          ...
          const shiftInspectors = buildingShifts.map((shift) => ({
